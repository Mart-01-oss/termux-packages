...
termux_setup_cabal() {
	if [[ "${TERMUX_ON_DEVICE_BUILD}" == "false" ]]; then
		...
		if [[ ! -d "${TERMUX_CABAL_RUNTIME_FOLDER}" ]]; then
		...
		# Ensure hackage index exists (cache restores may include cabal runtime but not the index).
		local idx_dir="$HOME/.cache/cabal/packages/hackage.haskell.org"
		if [[ "${TERMUX_PACKAGES_OFFLINE-false}" != "true" ]] && \
			[[ ! -f "$idx_dir/01-index.cache" ]] && [[ ! -f "$idx_dir/01-index.tar" ]] && [[ ! -f "$idx_dir/00-index.tar" ]]; then
			cabal update
		fi
	else
